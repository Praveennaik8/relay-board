...
export function SignInPage() {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#f8fafc] p-6"><section className="w-full max-w-md rounded-2xl border bg-white p-8 shadow-sm sm:p-10"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#1a73e8] text-white"><MessageSquareText className="h-6 w-6" /></div><h1 className="mt-7 text-3xl font-semibold tracking-tight text-slate-900">Keep work moving.</h1><p className="mt-3 text-sm leading-6 text-slate-600">RelayBoard is a shared, realtime space for the updates your team needs to see.</p><Button className="mt-8 w-full" size="lg" onClick={signIn} disabled={loading}>{loading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <GoogleMark />}Continue with Google</Button><p className="mt-5 text-center text-xs leading-5 text-slate-500">Sign in to find or create a board for your team.</p></section></main>;
}
